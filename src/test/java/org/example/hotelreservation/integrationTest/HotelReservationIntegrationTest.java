...
public class HotelReservationIntegrationTest {
    ...
    public void createUserAndRoom() throws Exception {
        String userJson = "{\"username\":\"testUser\",\"password\":\"testPass\",\"role\":\"ADMIN\"}";
    ...
    public void cleanupEach() throws Exception {
        if (reservationId != null) {
            mockMvc.perform(delete("/api/reservations/" + reservationId).with(user("admin").roles("ADMIN"))).andExpect(status().isOk());
            reservationId = null;
        }
        if (roomId != null) {
            mockMvc.perform(delete("/api/rooms/" + roomId).with(user("admin").roles("ADMIN"))).andExpect(status().isOk());
            roomId = null;
        }
        if (userId != null) {
            mockMvc.perform(delete("/api/users/" + userId).with(user("admin").roles("ADMIN"))).andExpect(status().isOk());
            userId = null;
        }
    }
    ...
    public void testFullReservationFlow() throws Exception {
        ...
        String resResp = mockMvc.perform(post("/api/reservations")
                            .with(user("admin").roles("ADMIN"))
                            .contentType(MediaType.APPLICATION_JSON)
                            ...
                            .getResponse()
                            .getContentAsString();
        reservationId = Long.valueOf(JsonPath.read(resResp, "$.id").toString());
    }
}